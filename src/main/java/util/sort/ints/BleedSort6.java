...
public class BleedSort6 {
    ...
    public static void bleedSort(int[] a)
    {
        ...
        int sampleSize = 160;
        int[] sample = sample(a, sampleSize);
        Arrays.sort(sample);
                ...
                || q[8] - q[0] < a.length >>> 1)
        {
            ...
            countingBleedSort(a, Ints.fill(tmpSize >> 1, Integer.MIN_VALUE), q, sampledRepetition);
            return;
        }
        else
        {
    ...
    private static int[] sample(int[] a, int sampleSize)
    {
    ...
    private static int[] nineQuantiles(int[] sample)
    {
...
//                    System.out.println("bleed");
    ...
    public static void countingBleedSort(int[] a)
    {
        int sampleSize = 160;
        int[] sample = sample(a, sampleSize);
        Arrays.sort(sample);
        int[] q = nineQuantiles(sample);
        int[] tmp = Ints.fill(a.length * 2, Integer.MIN_VALUE);
        countingBleedSort(a, tmp, sample, new double[]{1,1});
    }
    ...
    public static void countingBleedSort(int[] a, int[] tmp, int[] quantiles, double[] sampledRepetition)
    {
        ...
        double mult0 = (baseline1 - baseline0) / (double) Math.max(1, (q1 - q0));
        double mult1 = (baseline2 - baseline1) / (double) Math.max(1, (q2 - q1));
        double mult2 = (baseline3 - baseline2) / (double) Math.max(1, (q3 - q2));
        double mult3 = (baseline4 - baseline3) / (double) Math.max(1, (q4 - q3));
        double mult4 = (baseline5 - baseline4) / (double) Math.max(1, (q5 - q4));
        double mult5 = (baseline6 - baseline5) / (double) Math.max(1, (q6 - q5));
        double mult6 = (baseline7 - baseline6) / (double) Math.max(1, (q7 - q6));
        double mult7 = (baseline8 - baseline7) / (double) Math.max(1, (q8 - q7));
        ...
        for(int i = 0; i < a.length; i++)
        {
            ...
            else
            {
                ...
                if (place >= tmp.length)
                {
                    ...
                    if (maxHelpCount++ > a.length >> 4 && i < a.length >>> 1)
                    {   // problem in sample distribution (maximum was estimated too small)
                        fillPartiallyWithCounts(a, tmp, counts);
                        System.out.println("out-right");
                        tryOtherSort(quantiles, sampledRepetition, bleedCount, a);
